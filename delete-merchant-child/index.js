    ...
    try {
        if(!uuid.validate(req.params.parentMerchantID)){
            utils.setContextResError(
                ...
                new errors.InvalidUUIDError(
                    'The parentMerchantID field specified in the request params does not match the UUID v4 format.',
                    400
                )
            )
        }
        ...
        if(!uuid.validate(req.params.childID)){
            utils.setContextResError(
                context,
                new errors.InvalidUUIDError(
                    'The childID field specified in the request params does not match the UUID v4 format.',
                    400
                )
            )
        }
